use super::{BytePartErrorKind, ParseBytePartError};
use std::fmt;
use std::str::FromStr;
...
impl BytePart {
    pub fn any() -> Self {
        BytePart {
            value: BytePartValue::Any,
        }
    }

    pub fn low(byte: u8) -> Self {
        BytePart {
            value: BytePartValue::Low(low_nibble(byte)),
        }
    }

    pub fn high(byte: u8) -> Self {
        BytePart {
            value: BytePartValue::High(low_nibble(byte)),
        }
    }

    pub fn full(byte: u8) -> Self {
        BytePart {
            value: BytePartValue::Full(byte),
        }
    }

    pub fn parse(high: u8, low: u8) -> Result<BytePart, ParseBytePartError> {
        Ok(BytePart {
            value: BytePartValue::from_parts(
                try_get_nibble(Some(high))?,
                try_get_nibble(Some(low))?,
            ),
        })
    }
...
impl FromStr for BytePart {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if bytes.next().is_some() {
            Err(ParseBytePartError::from_kind(
                BytePartErrorKind::WrongLength,
            ))
        } else {
            Ok(BytePart {
                value: BytePartValue::from_parts(high, low),
            })
        }
    }
}
...
fn hexchar_to_nibble(hex: u8) -> Result<u8, ParseBytePartError> {
    match hex {
        ...
        _ => Err(ParseBytePartError::from_kind(
            BytePartErrorKind::InvalidChar(hex),
        )),
    }
}
...
fn try_get_nibble(c: Option<u8>) -> Result<Option<u8>, ParseBytePartError> {
    match c {
        None => Err(ParseBytePartError::from_kind(
            BytePartErrorKind::WrongLength,
        )),
...
mod bytepart_tests {
    ...
    fn bytepart_factory_low_downcast() {
        let byte = 0xD2_u8;
        assert_eq!(
            BytePartValue::Low(low_nibble(byte)),
            BytePart::low(byte).value
        );
    }
    ...
    fn bytepart_factory_high_downcast() {
        let byte = 0x2D_u8;
        assert_eq!(
            BytePartValue::High(low_nibble(byte)),
            BytePart::high(byte).value
        );
    }
    ...
    fn bytepart_factory_parse_ok_any() {
        assert_eq!(
            BytePartValue::Any,
            BytePart::parse(b'?', b'?').unwrap().value
        );
    }
    ...
    fn bytepart_factory_parse_ok_low() {
        assert_eq!(
            BytePartValue::Low(0x8),
            BytePart::parse(b'?', b'8').unwrap().value
        );
    }
    ...
    fn bytepart_factory_parse_ok_high() {
        assert_eq!(
            BytePartValue::High(0x0),
            BytePart::parse(b'0', b'?').unwrap().value
        );
    }
    ...
    fn bytepart_factory_parse_ok_full() {
        assert_eq!(
            BytePartValue::Full(0x54),
            BytePart::parse(b'5', b'4').unwrap().value
        );
    }
    ...
    fn bytepart_factory_parse_fail_low() {
        assert_eq!(
            Err(ParseBytePartError::from_kind(
                BytePartErrorKind::InvalidChar(b'Q')
            )),
            BytePart::parse(b'?', b'Q')
        );
    }
    ...
    fn bytepart_factory_parse_fail_high() {
        assert_eq!(
            Err(ParseBytePartError::from_kind(
                BytePartErrorKind::InvalidChar(b'Z')
            )),
            BytePart::parse(b'Z', b'?')
        );
    }
    ...
    fn bytepart_from_str_fail_invalid_high() {
        assert_eq!(
            Err(ParseBytePartError::from_kind(
                BytePartErrorKind::InvalidChar(b'!')
            )),
            "!D".parse::<BytePart>()
        );
    }
    ...
    fn bytepartvalue_from_parts_low() {
        assert_eq!(
            BytePartValue::Low(0x7),
            BytePartValue::from_parts(None, Some(0x7))
        );
    }
    ...
    fn bytepartvalue_from_parts_high() {
        assert_eq!(
            BytePartValue::High(0x4),
            BytePartValue::from_parts(Some(0x4), None)
        );
    }
    ...
    fn bytepartvalue_from_parts_full() {
        assert_eq!(
            BytePartValue::Full(0x98),
            BytePartValue::from_parts(Some(0x9), Some(0x8))
        );
    }
    ...
    fn parsebyteparterror_description_invalid_char() {
        let descr =
            ParseBytePartError::from_kind(BytePartErrorKind::InvalidChar(b'G')).description();
    ...
    fn parsebyteparterror_fmt_invalid_char() {
        let descr = format!(
            "{}",
            ParseBytePartError::from_kind(BytePartErrorKind::InvalidChar(b'G'))
        );
        assert_eq!("'G'!", &descr[descr.len() - 4..]);
    }
    ...
    fn parsebyteparterror_fmt_wrong_length() {
        let descr = format!(
            "{}",
            ParseBytePartError::from_kind(BytePartErrorKind::WrongLength)
        );
        assert_eq!("Wrong length!", &descr);
    }
    ...
    fn hexchar_to_nibble_fail1() {
        assert_eq!(
            Err(ParseBytePartError::from_kind(
                BytePartErrorKind::InvalidChar(b'S')
            )),
            hexchar_to_nibble(b'S')
        );
    }
    ...
    fn try_get_nibble_fail_char1() {
        assert_eq!(
            Err(ParseBytePartError::from_kind(
                BytePartErrorKind::InvalidChar(b'w')
            )),
            try_get_nibble(Some(b'w'))
        );
    }
    ...
    fn try_get_nibble_fail_char2() {
        assert_eq!(
            Err(ParseBytePartError::from_kind(
                BytePartErrorKind::InvalidChar(b'K')
            )),
            try_get_nibble(Some(b'K'))
        );
    }
}
