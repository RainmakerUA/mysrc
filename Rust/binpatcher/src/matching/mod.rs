...
pub fn matches_from_slice<'a, OU: Into<Option<usize>>>(
    pattern: &'a Pattern,
    bytes: &'a [u8],
    start_pos: OU,
) -> PatternMatchIterator<'a> {
    if pattern.is_empty() {
...
pub fn matches_from_reader<R: Read, OU: Into<Option<usize>>>(
    pattern: &Pattern,
    reader: R,
    buffer_size: OU,
) -> ReaderMatchIterator<'_, R> {
    if pattern.is_empty() {
        panic!("Cannot match on empty pattern!")
    }

    let bsi = BufferedSliceIterator::new(
        reader,
        pattern.len(),
        1,
        buffer_size.into().unwrap_or(DEFAULT_BUFFER_SIZE),
    );
